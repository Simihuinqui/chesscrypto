...
from chesscrypto import Variants
...
class NormalGameType(GameType):
    def __init__(self, fics_name, short_fics_name, rating_type, display_text):
        GameType.__init__(
            self, fics_name, short_fics_name, rating_type, display_text=display_text
        )
...
class VariantGameType(GameType):
    def __init__(self, fics_name, short_fics_name, rating_type, variant_type):
        GameType.__init__(
            self, fics_name, short_fics_name, rating_type,
        )
...
class WildGameType(VariantGameType):
    ...
    def __init__(self, fics_name, variant_type):
        VariantGameType.__init__(
            self, fics_name, "w", TYPE_WILD, variant_type=variant_type
        )
...
GAME_TYPES = {
    ...
    "45-minute": NormalGameType(
        "45-minute", "J", TYPE_FORTYFIVE_MINUTE, _("45-minute")
    ),
...
for key in GAME_TYPES:
    if not isinstance(GAME_TYPES[key], WildGameType):
        GAME_TYPES_BY_SHORT_FICS_NAME[GAME_TYPES[key].short_fics_name] = GAME_TYPES[key]
...
(
    ...
    TYPE_UNREGISTERED,
    TYPE_CHESS_ADVISOR,
    TYPE_SERVICE_REPRESENTATIVE,
    TYPE_TOURNAMENT_DIRECTOR,
    TYPE_MAMER_MANAGER,
    TYPE_GRAND_MASTER,
    ...
    TYPE_DUMMY_ACCOUNT,
    TYPE_CANDIDATE_MASTER,
    TYPE_FIDE_ARBEITER,
    TYPE_NATIONAL_MASTER,
    TYPE_DISPLAY_MASTER,
) = range(20)
...
TITLES = {  # From FICS 'help who'
    ...
    "WCM": TYPE_WOMAN_CANDIDATE_MASTER,
    "NM": TYPE_NATIONAL_MASTER,
    "XM": TYPE_CRYPTO_MASTER,
}
...
def get_infobarmessage_content2(player, heading_text, message_text, gametype=None):
    hbox = Gtk.HBox()
    image = Gtk.Image()
