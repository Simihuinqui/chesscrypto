...
from chesscrypto import VERSION
...
from chesscrypto.Utils.lutils.lmove import parseAny, toSAN
...
class InternetGameInterface:
    ...
    def __init__(self):
        ...
        self.userAgent = "ChessCrypto %s" % VERSION
    ...
    def is_enabled(self):
        ...
        return (proto != CAT_WS) or (
            sys.version_info.major >= 3 and sys.version_info.minor >= 5
        )  # Async WS needs 3.5 for SSL
    ...
    def is_fen(self, fen):
        ...
        try:
            rxp = re.compile(
                r"^[kqbnrp1-8\/]+\s[w|b]\s[kq-]+\s[a-h-][1-8]?(\s[0-9]+)?(\s[0-9]+)?$",
                re.IGNORECASE,
            )
...
class InternetGameLichess(InternetGameInterface):
    ...
    def assign_game(self, url):
        ...
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?lichess\.(org|dev)\/broadcast\/[a-z0-9\-]+\/([a-z0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
        ...
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?lichess\.(org|dev)\/practice\/[\w\-\/]+\/([a-z0-9]+\/[a-z0-9]+)(\.pgn)?\/?([\S\/]+)?$",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
        ...
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?lichess\.(org|dev)\/study\/([a-z0-9]+(\/[a-z0-9]+)?)(\.pgn)?\/?([\S\/]+)?$",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
        ...
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?lichess\.(org|dev)\/training\/([0-9]+|daily)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
        ...
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?lichess\.(org|dev)\/(game\/export\/|embed\/)?([a-z0-9]+)\/?([\S\/]+)?$",
            re.IGNORECASE,
        )  # More permissive
        m = rxp.match(url)
        if m is not None:
    ...
    def download_game(self):
        ...
        if self.url_type == TYPE_GAME:
            ...
            game = {}
            game["_url"] = "https://lichess.{}{}".format(
                self.url_tld, self.json_field(api, "url/round")
            )
            game["Variant"] = self.json_field(api, "game/variant/name")
            game["FEN"] = self.json_field(api, "game/initialFen")
            game["SetUp"] = "1"
            game["White"] = self.json_field(
                api,
                "player/name",
                self.json_field(api, "player/user/username", "Anonymous"),
            )
            game["WhiteElo"] = self.json_field(api, "player/rating")
            game["Black"] = self.json_field(
                api,
                "opponent/name",
                self.json_field(api, "opponent/user/username", "Anonymous"),
            )
            game["BlackElo"] = self.json_field(api, "opponent/rating")
            if self.json_field(api, "clock") != "":
                game["TimeControl"] = "%d+%d" % (
                    self.json_field(api, "clock/initial"),
                    self.json_field(api, "clock/increment"),
                )
            else:
                game["TimeControl"] = "%dd" % (
                    self.json_field(api, "correspondence/increment") // 86400
                )
            game["Result"] = "*"
            game["_moves"] = ""
        ...
        # Logic for the puzzles
            ...
            game["_url"] = "https://lichess.{}/{}#{}".format(
                self.url_tld,
                self.json_field(puzzle, "gameId"),
                self.json_field(puzzle, "initialPly"),
            )
            game["Site"] = "lichess.%s" % self.url_tld
            rating = self.json_field(puzzle, "rating")
            game["Event"] = "Puzzle %d, rated %s" % (
                self.json_field(puzzle, "id"),
                rating,
            )
            game["Result"] = "*"
...
class InternetGameChessgames(InternetGameInterface):
    ...
    def download_game(self):
        ...
        url = "http://www.chessgames.com/pgn/chesscrypto.pgn?gid=" + self.id
        if self.computer:
...
class InternetGameFicsgames(InternetGameInterface):
    ...
    def assign_game(self, url):
        ...
        parsed = urlparse(url)
        if (
            parsed.netloc.lower() not in ["www.ficsgames.org", "ficsgames.org"]
            or "show" not in parsed.path.lower()
        ):
            return False
    ...
    def download_game(self):
        ...
        # Download
        pgn = self.download(
            "http://ficsgames.org/cgi-bin/show.cgi?ID=%s;action=save" % self.id
        )
...
class InternetGameChesstempo(InternetGameInterface):
    ...
    def assign_game(self, url):
        # Puzzles
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?chesstempo\.com\/chess-tactics\/(\d+)", re.IGNORECASE
        )
        m = rxp.match(url)
        if m is not None:
            gid = str(m.group(2))
        ...
        # Games
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?chesstempo\.com\/gamedb\/game\/(\d+)", re.IGNORECASE
        )
        m = rxp.match(url)
        if m is not None:
            gid = str(m.group(2))
    ...
    async def download_game(self):
        ...
        if self.url_type == TYPE_GAME:
            pgn = self.download(
                "http://chesstempo.com/requests/download_game_pgn.php?gameids=%s"
                % self.id,
                userAgent=True,
            )  # Else a random game is retrieved
        ...
        # Puzzles
            ...
            async def coro(self):
                ...
                ws = await websockets.connect(
                    "wss://chesstempo.com:443/ws",
                    origin="https://chesstempo.com",
                    extra_headers=[("User-agent", self.userAgent)],
                    ping_interval=None,
                )
                try:
                    ...
                    if (data["eventName"] == "connectionStarted") and (
                        data["data"] == "started"
                    ):
                        # Call the puzzle
                        await ws.send(
                            '{"eventName":"get-problem-session-data","data":{"problemSetId":1,"sessionSize":20}}'
                        )
                        await ws.send(
                            '{"eventName":"set-problem-difficulty","data":{"difficulty":"","problemSetId":1}}'
                        )
                        await ws.send(
                            '{"eventName":"get-tactic","data":{"problemId":%s,"vo":false}}'
                            % self.id
                        )
            ...
            game["Event"] = "Puzzle %s" % self.json_field(
                puzzle, "tacticInfo/problem_id"
            )
            game["White"] = _("White")
            game["Black"] = _("Black")
            game["Result"] = "*"
            ...
            game["SetUp"] = "1"
            game["_moves"] = "{{{}}} {}".format(
                self.json_field(puzzle, "tacticInfo/prevmove"),
                self.json_field(puzzle, "tacticInfo/moves"),
            )
...
class InternetGameChessOrg(InternetGameInterface):
    ...
    def assign_game(self, url):
        rxp = re.compile(
            r"^https?:\/\/chess\.org\/play\/([a-f0-9\-]+)[\/\?\#]?", re.IGNORECASE
        )
        m = rxp.match(url)
        if m is not None:
            id = str(m.group(1))
    ...
    async def download_game(self):
        ...
        async def coro(self):
            ...
            ws = await websockets.connect(
                url, origin="https://chess.org:443", ping_interval=None
            )
            try:
                # Server: Hello
                data = await ws.recv()
...
class InternetGameEuropeechecs(InternetGameInterface):
    ...
    def download_game(self):
        ...
        if self.id.lower().endswith(".pgn"):
            ...
            rxp = re.compile(
                r".*class=\"cbwidget\"\s+id=\"([0-9a-f]+)_container\".*", re.IGNORECASE
            )
            lines = page.split("\n")
            for line in lines:
                m = rxp.match(line)
                if m is not None:
                    links.append(
                        "https://www.europe-echecs.com/embed/doc_%s.pgn" % m.group(1)
                    )
...
class InternetGameChessCom(InternetGameInterface):
    ...
    def assign_game(self, url):
        ...
        # Puzzles
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?chess\.com\/([a-z\/]+)?(puzzles)\/problem\/([0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
        ...
        # Games
        url = url.replace("/live#g=", "/live/game/").replace(
            "/daily#g=", "/daily/game/"
        )
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?chess\.com\/([a-z\/]+)?(live|daily)\/([a-z\/]+)?([0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
    ...
    def download_game(self):
        ...
        # Puzzles
            ...
            rating = self.json_field(puzzle, "rating")
            game["Event"] = "Puzzle {}, rated {}".format(
                str(self.json_field(puzzle, "id")), rating
            )
            game["White"] = _("White")
            game["Black"] = _("Black")
            game["Result"] = "*"
        ...
        # Games
            ...
            if (
                ...
                and self.json_field(chessgame, "game/isRated")
                and not self.json_field(chessgame, "game/isFinished")
            ):
                return None
...
class InternetGameSchachspielen(InternetGameInterface):
    ...
    def assign_game(self, url):
        rxp = re.compile(
            r"^https?:\/\/(www\.)?schach-spielen\.eu\/(game|analyse)\/([a-z0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
...
class InternetGameRedhotpawn(InternetGameInterface):
    ...
    def download_game(self):
        ...
        if self.url_type == TYPE_GAME:
            url = (
                "https://www.redhotpawn.com/pagelet/view/game-pgn.php?gameid=%s"
                % self.id
            )
        elif self.url_type == TYPE_PUZZLE:
            if "://" in self.id:
                ...
                url = (
                    "https://www.redhotpawn.com/chess-puzzles/chess-puzzle-solve.php?puzzleid=%s"
                    % self.id
                )
...
class InternetGameSchacharena(InternetGameInterface):
    ...
    def assign_game(self, url):
        ...
        parsed = urlparse(url)
        if (
            parsed.netloc.lower() not in ["www.schacharena.de", "schacharena.de"]
            or "verlauf" not in parsed.path.lower()
        ):
            return False
    ...
    def download_game(self):
        ...
        # Download page
        page = self.download(
            "https://www.schacharena.de/new/verlauf.php?brett=%s" % self.id
        )
        if page is None:
            return None

        # Init
        rxp_player = re.compile(
            r".*spielerstatistik.*name=(\w+).*\[([0-9]+)\].*", re.IGNORECASE
        )
        rxp_move = re.compile(r".*<span.*onMouseOut.*fan\(([0-9]+)\).*", re.IGNORECASE)
        rxp_result = re.compile(
            r".*>(1\-0|0\-1|1\/2\-1\/2)\s([^\<]+)<.*", re.IGNORECASE
        )
        player_count = 0
        board = LBoard()
        board.applyFen(FEN_START)
        ...
        game["_moves"] = ""
        game["_url"] = (
            "https://www.schacharena.de/new/verlauf_to_pgn_n.php?brett=%s" % self.id
        )  # If one want to get the full PGN
        lines = page.split("\n")
        for line in lines:
            # Player
            ...
            if m is not None:
                move = m.group(1)
                move = (
                    "_abcdefgh"[int(move[0])]
                    + move[1]
                    + "_abcdefgh"[int(move[2])]
                    + move[3]
                )
                if self.use_an:
                    kmove = parseAny(board, move)
...
class InternetGameChesspuzzle(InternetGameInterface):
    ...
    def assign_game(self, url):
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?chesspuzzle\.net\/(Puzzle|Solution)\/([0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
    ...
    def download_game(self):
        ...
        # Download the puzzle
        page = self.download(
            "https://chesspuzzle.net/Solution/%s" % self.id, userAgent=True
        )  # Else 403 Forbidden
        if page is None:
            return None
...
class InternetGameChessking(InternetGameInterface):
    ...
    def assign_game(self, url):
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?chessking\.com\/games\/(ff\/)?([0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
...
class InternetGameIdeachess(InternetGameInterface):
    ...
    def assign_game(self, url):
        ...
        if m is not None:
            ...
            if gid.isdigit() and gid != "0":
                # Game type
                classification = [
                    ("/chess_tactics_puzzles/checkmate_n/", "m"),
    ...
    def download_game(self):
        ...
        if self.url_type == "m":
            game["_url"] = (
                "http://www.ideachess.com/chess_tactics_puzzles/checkmate_n/%s"
                % self.id
            )
        elif self.url_type == "t":
            game["_url"] = (
                "http://www.ideachess.com/chess_tactics_puzzles/tactics_n/%s" % self.id
            )
        else:
            assert False
...
class InternetGameChessdb(InternetGameInterface):
    ...
    def assign_game(self, url):
        ...
        parsed = urlparse(url)
        if (
            parsed.netloc.lower() not in ["www.chess-db.com", "chess-db.com"]
            or "game.jsp" not in parsed.path.lower()
        ):
            return False
...
class InternetGameChesspro(InternetGameInterface):
    ...
    def download_game(self):
        ...
        rxp = re.compile(
            r'.*OpenGame\(\s*"g[0-9]+\"\s*,"(.*)"\s*\)\s*;.*', re.IGNORECASE
        )
        lines = page.split("\n")
        for line in lines:
            m = rxp.match(line)
...
class InternetGameFicgs(InternetGameInterface):
    ...
    def assign_game(self, url):
        rxp = re.compile(
            r"^https?:\/\/(\S+\.)?ficgs\.com\/game_(\d+).html", re.IGNORECASE
        )
        m = rxp.match(url)
        if m is not None:
            gid = str(m.group(2))
...
class InternetGamePychess(InternetGameInterface):
    ...
    def assign_game(self, url):
        # Retrieve the ID of the game
        rxp = re.compile(
            r"https?:\/\/(www\.)?chesscrypto(-variants\.herokuapp\.com|\.org)\/([a-z0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
    ...
    async def download_game(self):
        ...
        async def coro(self):
            result = None
            ws = await websockets.connect(
                "wss://www.chesscrypto.org/wsr",
                origin="https://www.chesscrypto.org",
                ping_interval=None,
            )
            try:
...
class InternetGameGeneric(InternetGameInterface):
    ...
    def download_game(self):
        ...
        # Chess file
        if (mime in ["application/x-chess-pgn", "application/pgn"]) or (
            self.allow_octet_stream and mime == "application/octet-stream"
        ):
            return data
