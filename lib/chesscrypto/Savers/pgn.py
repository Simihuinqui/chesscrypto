...
from chesscrypto.external.scoutfish import Scoutfish
from chesscrypto.external.chess_db import Parser
...
from chesscrypto.System.protoopen import PGN_ENCODING
from chesscrypto.System.prefix import getEngineDataPrefix
from chesscrypto.Utils.lutils.LBoard import LBoard
from chesscrypto.Utils.GameModel import GameModel
from chesscrypto.Utils.lutils.lmove import toSAN, parseAny, ParsingError
from chesscrypto.Utils.Move import Move
from chesscrypto.Utils.elo import get_elo_rating_change_pgn
from chesscrypto.Utils.logic import getStatus
from chesscrypto.Variants import name2variant, NormalBoard, variants
from chesscrypto.Utils import formatTime
from chesscrypto.Savers.ChessFile import ChessFile, LoadingError
from chesscrypto.Savers.database import col2label, TagDatabase, parseDateTag
from chesscrypto.System.cpu import get_cpu
from chesscrypto.Database import model as dbmodel
from chesscrypto.Database.PgnImport import TAG_REGEX, pgn2Const, PgnImport
from chesscrypto.Database.model import (
    game,
    create_indexes,
    drop_indexes,
    metadata,
    ...
    if indented:
        ...
        for text in result:
            ...
            if (
                ...
                and text != ")"
                and len(text_buffer) > 0
            ):
                text_buffer += " "
            # New line for a new main move
            if len(text_buffer) == 0 or (
                indented
                and depth == 0
                and last != "\r"
                and last != "\n"
...
if sys.platform == "win32":
    ...
    scoutfish = "scoutfish_x{}{}{}".format(
        cpuinfo["bitness"], MODERN, cpuinfo["binext"]
    )
    ...
    def init_chess_db(self):
        ...
        if chess_db_path is not None and self.path and self.size > 0:
            try:
                ...
                self.chess_db = None
                log.warning(
                    "Failed to sart chess_db parser. OSError {} {}".format(
    ...
    def init_scoutfish(self):
        ...
        if scoutfish_path is not None and self.path and self.size > 0:
            try:
                ...
                self.scoutfish = None
                log.warning(
                    "Failed to sart scoutfish. OSError {} {}".format(
                        err.errno, err.strerror
                    )
                )
            except pexpect.TIMEOUT:
