...
from chesscrypto.Utils import prettyPrintScore
...
from chesscrypto.Utils.lutils.lmove import toSAN, toFAN, FCORD, TCORD
from chesscrypto.Savers.pgn import move_count, nag2symbol, parseTimeControlTag
from chesscrypto.widgets.ChessClock import formatTime
from chesscrypto.widgets.LearnInfoBar import LearnInfoBar
from chesscrypto.widgets import insert_formatted, preferencesDialog, mainwindow
from chesscrypto.widgets.Background import isDarkTheme
...
class Sidepanel:
    ...
    def menu_edit_comment(self, widget=None, board=None, index=0):
        ...
        comment = (
            ...
            else textbuffer.get_text(iter_first, iter_last, False)
        )
    ...
    def apply_symbols(self, text):
        ...
        if self.fan:
            re_decoration = re.compile(
                r"^([^a-hprnkqx1-8]*|[0-9]+\.+)?([a-hprnkqx1-8=@]+)([^a-hprnkqx1-8]*)$",
                re.IGNORECASE,
            )
            re_move = re.compile(
    ...
    def __movestr(self, board):
        ...
        return "{}{}{}".format(move_count(board), movestr.replace("-", "⁃"), nagsymbols)
