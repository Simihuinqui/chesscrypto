...
from chesscrypto.Utils import wait_signal
...
from chesscrypto.Utils.Cord import Cord
from chesscrypto.Utils.Move import toSAN, toAN, parseAny
from chesscrypto.Utils.Offer import Offer
...
from chesscrypto.Utils.logic import validate, getMoveKillingKing
from chesscrypto.Utils.lutils.ldata import MATE_VALUE
from chesscrypto.Utils.lutils.lmove import ParsingError
from chesscrypto.Variants import variants
from chesscrypto.Players.Player import PlayerIsDead, TurnInterrupt, InvalidMove
...
class CECPEngine(ProtocolEngine):
            ...
            try:
                ...
                if return_value == "not ready":
                    return_value = await asyncio.wait_for(
                        self.queue.get(), TIME_OUT_SECOND
                    )
    ...
    def __onReadyForMoves(self, self_):
        if self.mode in (ANALYZING, INVERSE_ANALYZING):
            ...
            # http://code.google.com/p/chesscrypto/issues/detail?id=515
    ...
    def setOptionVariant(self, variant):
        if self.features["variants"] is None:
            log.warning(
                "setOptionVariant: engine doesn't support variants",
                extra={"task": self.defname},
            )
            return
    ...
    def setOptionStrength(self, strength, forcePonderOff):
        ...
        if strength <= 19:
            self.__setTimeHandicap(0.01 * 10 ** (strength / 10.0))
    ...
    def playerUndoMoves(self, moves, gamemodel):
        log.debug(
            "CECPEngine.playerUndoMoves: moves=%s self=%s gamemodel.curplayer=%s"
            % (moves, self, gamemodel.curplayer),
            extra={"task": self.defname},
        )
        ...
        if (
            gamemodel.curplayer != self
            and moves % 2 == 1
            or (gamemodel.curplayer == self and moves % 2 == 0)
        ):
            # Interrupt if we were searching, but should no longer do so
            log.debug(
                "CECPEngine.playerUndoMoves: putting TurnInterrupt into self.move_queue %s"
                % self.name,
                extra={"task": self.defname},
            )
            ...
            if not line:
                ...
                if self.board:
                    ...
                    if movestr:
                        ...
                        if self.engineIsInNotPlaying:
                            ...
                            log.info(
                                "__parseLine: Discarding engine's move: %s" % movestr,
                                extra={"task": self.defname},
                            )
                            print("undo", file=self.engine)
                            continue
                ...
                if self.engineIsInNotPlaying:
                    ...
                    if match:
                        ...
                        nps = (
                            str(int(int(nodes) / (int(time) / 100)))
                            if int(time) > 0
                            else ""
                        )
                ...
                if parts[0] == "resign" or (
                    parts[0] == "tellics" and parts[1] == "resign"
                ):  # buggy crafty
                ...
                # Tell Somebody
                if parts[0][:4] == "tell" and parts[0][4:] in (
                    "others",
                    "all",
                    "ics",
                    "icsnoalias",
                    ...
                    for i, pair in enumerate(parts[1:]):
                        ...
                        if key == "done":
                            if value == 1:
                                ...
                                log.info(
                                    "Adds %d seconds timeout" % TIME_OUT_SECOND,
                                    extra={"task": self.defname},
                                )
