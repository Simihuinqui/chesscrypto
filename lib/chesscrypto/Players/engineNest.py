...
from chesscrypto.System.command import Command
from chesscrypto.System.SubProcess import SubProcess
from chesscrypto.System.prefix import (
    addUserConfigPrefix,
    getDataPrefix,
    getEngineDataPrefix,
)
from chesscrypto.Players.Player import PlayerIsDead
from chesscrypto.Utils import createStoryTextAppEvent
...
from chesscrypto.Players.engineList import PYTHONBIN, VM_LIST, ENGINES_LIST
...
class EngineDiscoverer(GObject.GObject):
    ...
    def __findRunData(self, engine):
        ...
        For the ChessCrypto engine, special handling is taken, and we search
        ...
        if engine.get("vm_name") is not None:
            ...
            if engine["name"] == "ChessCrypto.py" and not getattr(sys, "frozen", False):
                path = join(abspath(dirname(__file__)), "ChessCrypto.py")
    ...
    def __clean(self, rundata, engine):
        ...
        if refeng is None:
            log.warning(
                "Engine '%s' is not in ChessCrypto predefined known engines list"
                % engine.get("name")
            )
            engine["recheck"] = True
        else:
    ...
    def addEngine(self, name, new_engine, protocol, vm_name, vm_args):
        ...
        engine["workingDirectory"] = os.path.dirname(new_engine)
        engine["recheck"] = True
