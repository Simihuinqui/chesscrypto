...
from chesscrypto.System.repeat import repeat_sleep
from chesscrypto.System import fident
...
from chesscrypto.Utils.lutils.lmove import determineAlgebraicNotation, toLAN, parseSAN
from chesscrypto.Utils.lutils import lsearch
from chesscrypto.Utils.repr import reprResult_long, reprReason_long
from chesscrypto.ic.FICSConnection import FICSMainConnection


class ChessCryptoFICS(ChessCrypto):
    def __init__(self, password, from_address, to_address):
        ...
        if not password:
            ...
            self.username = "ChessCrypto"
        self.owner = "Lobais"
        self.password = password
        self.from_address = "The ChessCrypto Bot <%s>" % from_address
        ...
        # The amount of random challenges, that ChessCrypto sends with each seek
        self.challenges = 10
        # enableEGTB()
    ...
    def makeReady(self):
        ...
        self.connection.fm.setFingerNote(
            1,
            "ChessCrypto is the chess engine bundled with the ChessCrypto %s " % chesscrypto.VERSION
        ...
        self.connection.fm.setFingerNote(
            2,
            "ChessCrypto is 100% Python code and is released under the terms of "
        ...
        self.connection.fm.setFingerNote(
            3,
            "ChessCrypto runs on an elderly AMD Sempron(tm) Processor 3200+, 512 "
        ...
        self.connection.fm.setFingerNote(
            4,
            "ChessCrypto uses a small 500 KB openingbook based solely on Kasparov "
        ...
        self.connection.fm.setFingerNote(
            5,
            "ChessCrypto will allow any pause/resume and adjourn wishes, but will "
            + "deny takebacks. Draw, abort and switch offers are accepted, "
            + "if they are found to be an advance. Flag is auto called, but "
            + "ChessCrypto never resigns. We don't want you to forget your basic "
            + "mating skills.",
        )
    ...
    def __usage(self):
        return (
            "|| ChessCrypto bot help file || "
            + "# help 'Displays this help file' "
            + "# sudo <password> <command> 'Lets ChessCrypto execute the given command' "
            + "# sendlog 'Makes ChessCrypto send you its current log'"
        )
    ...
    def __onTell(self, chatManager, name, title, isadmin, text):
        ...
        if args[0] == "sudo":
            ...
            if self.ownerOnline:
                self.tellHome(f"{name} told me '{text}'")
            else:
    ...
    def __go(self):
        ...
        #     lambda worker: ChessCrypto._ChessCrypto__go(self, worker))
...
if __name__ == "__main__":
    if len(sys.argv) == 5 and sys.argv[1] == "fics":
        chesscrypto_fics = ChessCryptoFICS(*sys.argv[2:])
    else:
        print("Unknown argument(s):", repr(sys.argv))
        sys.exit(0)

    chesscrypto_fics.makeReady()
    chesscrypto_fics.run()
