"""
This module provides some basic functions for accessing chesscrypto datefiles in
...
if getattr(sys, "frozen", False):
    _prefix = os.path.join(os.path.dirname(sys.executable), "share", "chesscrypto")
    _installed = True
else:
    ...
    if _path_is_child_of(sys.prefix, __file__):
        for sub in (
            ...
            _prefix = os.path.join(sys.prefix, sub, "chesscrypto")
            if os.path.isdir(os.path.join(_prefix, "pieces")):
                _installed = True
                break
        else:
            raise Exception("can't find the chesscrypto data directory")
    elif _path_is_child_of(home_local, __file__):
        _prefix = os.path.join(home_local, "share", "chesscrypto")
        if os.path.isdir(os.path.join(_prefix, "pieces")):
            _installed = True
        else:
            raise Exception("can't find the chesscrypto data directory")
...
chesscrypto = "chesscrypto"


def getUserDataPrefix():
    return os.path.join(get_user_data_dir(), chesscrypto)
...
def getUserConfigPrefix():
    return os.path.join(get_user_config_dir(), chesscrypto)
...
def getUserCachePrefix():
    return os.path.join(get_user_cache_dir(), chesscrypto)
