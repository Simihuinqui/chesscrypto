    ...
    class WinsoundPlayer(Player):
        ...
        def play(self, uri):
            try:
                ...
                log.error(
                    "ERROR: RuntimeError while playing %s." % url2pathname(uri[5:])
                )
        ...
        def __init__(self):
            ...
            try:
                if getattr(sys, "frozen", False):
                    ...
                    gst_player = os.path.join(
                        os.path.abspath(os.path.dirname(__file__)), "gst_player.py"
                    )
                self.player = subprocess.Popen(
                    [PYTHONBIN, gst_player], stdin=subprocess.PIPE
                )

                self.stdin = io.TextIOWrapper(
                    self.player.stdin, encoding="utf-8", line_buffering=True
                )
                self.ready = True
            except Exception:
                self.player = None
