...
from chesscrypto.widgets.InfoPanel import Panel
...
class ViewsPanel(Gtk.Notebook, Panel):
    ...
    def __init__(self, connection):
        ...
        label = Gtk.Label()
        label.set_markup("<big>%s</big>" % _("You have opened no conversations yet"))
    ...
    def addItem(self, grp_id, name, grp_type, chat_view):
        ...
        self.connection.cm.connect(
            "channelMessage", self.onChannelMessage, grp_id, chat_view
        )
        self.connection.cm.connect(
            "privateMessage", self.onPersonMessage, get_playername(name), chat_view
        )

        if grp_type == TYPE_CHANNEL:
            self.connection.cm.connect(
                "channelLog", self.onChannelLog, grp_id, chat_view
            )
            self.connection.cm.getChannelLog(grp_id)
            if not self.connection.cm.mayTellChannel(grp_id):
                chat_view.disable(_("Only registered users may talk to this channel"))
    ...
    def onChannelMessage(
        self, cm, name, isadmin, isme, channel, text, name_, chat_view
    ):
