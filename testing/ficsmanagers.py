...
from chesscrypto.ic.FICSObjects import (
    FICSPlayer,
    FICSGames,
    FICSSeeks,
    FICSChallenges,
...
from chesscrypto.ic import BLOCK_START, BLOCK_SEPARATOR, BLOCK_END
from chesscrypto.ic.FICSConnection import Connection
from chesscrypto.ic.VerboseTelnet import PredictionsTelnet, TelnetLine, BL
from chesscrypto.ic.managers.AdjournManager import AdjournManager
from chesscrypto.ic.managers.SeekManager import SeekManager
from chesscrypto.ic.managers.BoardManager import BoardManager
from chesscrypto.ic.managers.OfferManager import OfferManager
from chesscrypto.ic.managers.HelperManager import HelperManager
from chesscrypto.ic.managers.ErrorManager import ErrorManager
from chesscrypto.ic.managers.FingerManager import FingerManager
from chesscrypto.ic.managers.NewsManager import NewsManager
from chesscrypto.ic.managers.ChatManager import ChatManager
from chesscrypto.ic.managers.ConsoleManager import ConsoleManager
from chesscrypto.ic.managers.AutoLogOutManager import AutoLogOutManager
...
class AdjournManagerTests(EmittingTestCase):
    ...
    def test1(self):
        ...
        game = FICSAdjournedGame(
            ...
            time=gametime,
            rated=True,
            game_type=GAME_TYPES_BY_FICS_NAME["wild"],
            private=False,
            minutes=2,
            inc=2,
        )
    ...
    def test2(self):
        ...
        game2 = FICSAdjournedGame(
            FICSPlayer(self.connection.getUsername()),
            FICSPlayer("chesscrypto"),
            our_color=WHITE,
            length=4,
            time=gametime2,
            rated=False,
...
class SeekManagerTests(EmittingTestCase):
    ...
    def test7(self):
        ...
        """
        lines = [
            BLOCK_START + "54" + BLOCK_SEPARATOR + "156" + BLOCK_SEPARATOR,
            "<sr> 8 17 30",
...
class BoardManagerTests(EmittingTestCase):
    def setUp(self):
        ...
        def pr_handler(manager, offer):
            self.deleted_offers.add(offer)

        self.connection.om.connect("onChallengeRemove", pr_handler)

        self.deleted_seeks = set()

        def sr_handler(manager, seek):
    ...
    def match_offer(self, offer):
        return (
            "<pf> %s w=GuestABCD t=match p=GuestABCD (----) [black] GuestEFGH (----) unrated untimed"
            % offer
        )
    ...
    def test7(self):
        ...
        game = FICSGame(
            ...
            board=FICSBoard(
                300000,
                300000,
                fen="knnrrrqn pppppppp -------- -------- -------- -------- PPPPPPPP NRNKNQRR W -1 0 0 0 0 0",
            ),
        )
        me.game = game
        opponent.game = game
...
class GamesTests(EmittingTestCase):
    ...
    def test1(self):
        ...
        lines = [
            BLOCK_START + "218" + BLOCK_SEPARATOR + "80" + BLOCK_SEPARATOR,
...
class ConsoleManagerTests(EmittingTestCase):
    ...
    def test1(self):
        ...
        lines = [
            ...
            "2: If you're using Linux check out chesscrypto: http://www.chesscrypto.org",
            BLOCK_END,
        ]
